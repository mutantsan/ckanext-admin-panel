...
class ReportLogsView(MethodView):
    ...
    def _filter_items(self, item_list: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        if self.levels:
            item_list = [
                item for item in item_list if item["level"] in self.levels]
    ...
    def _get_table_columns(self) -> list[dict[str, Any]]:
        return [
            ...
            tk.h.ap_table_column(
                "level", column_renderer="ap_log_level", width="5%"),
            tk.h.ap_table_column(
                "timestamp", column_renderer="ap_date", width="10%"),
...
ap_report.add_url_rule(
    "/reports/logs", view_func=ReportLogsView.as_view("logs"))
